...
// Cartel lists in the API that bundled two canonical parties on one list.
// We split their seats/votes proportionally using fixed historical ratios
// (approximate; refined where the split is documented).
export const CARTEL_SPLITS: Record<string, Array<{ id: string; share: number }>> = {
	'CD&V NVA': [
		{ id: 'cdv', share: 0.7 },
		{ id: 'nva', share: 0.3 }
	], // 2007 cartel: 30 seats -> ~CD&V 23, N-VA ~5 (N-VA had 5)
	'sp.a-spirit': [
		{ id: 'spa', share: 0.92 },
		{ id: 'other', share: 0.08 }
	],
	...
	'FDF-PLDP': [{ id: 'fdf', share: 1.0 }],
	'PSB-RW': [{ id: 'ps', share: 1.0 }],
	'VU-ID': [{ id: 'vu', share: 1.0 }]
};

// Maps a raw API party label to a canonical party id. Returns 'other' for
// minor/unrecognized lists. Cartels are handled separately via CARTEL_SPLITS.
export function normalizeLabel(raw: string): string {
	...
	const map: Record<string, string> = {
		...
		'LES ENGAGES': 'engages',
		// socialists
		'SOCIALISTISCHE PARTIJ/ P': 'sp-unitary',
		'BSP/PSB': 'sp-unitary',
		'PSB-BSP': 'sp-unitary',
		BSP: 'sp',
		PSB: 'ps',
		...
		PS: 'ps',
		// liberals
		'PARTI LIBERALE/LIBERALE': 'lib-unitary',
		'PVV/PLP': 'lib-unitary',
		PLP: 'lib-unitary',
		'PLP(BRUX).': 'lib-unitary',
		PVV: 'pvv',
		VLD: 'vld',
		OPENVLD: 'openvld',
		'OPEN VLD': 'openvld',
		...
		ECOLO: 'ecolo',
		// far right
		'VLAAMS BLOK': 'vlaamsblok',
		'VLAAMS BELANG': 'vlaamsbelang',
		FN: 'fn',
		// far left
		'KPB/PCB': 'kpb',
		PCB: 'kpb',
